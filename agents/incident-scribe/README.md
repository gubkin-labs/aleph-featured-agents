...
Build an evidence-backed incident timeline and draft updates and a postmortem.
...
`0 */2 * * *` in UTC. Scheduled runs are read-only and produce reports or drafts only. Change or remove the schedule after cloning as needed.
...
1. Clone the agent and connect sentry, pagerduty, notion.
