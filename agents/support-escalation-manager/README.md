...
Identify urgent support cases and prepare engineering escalations and team alerts.
...
`0 */4 * * *` in UTC. Scheduled runs are read-only and produce reports or drafts only. Change or remove the schedule after cloning as needed.
...
1. Clone the agent and connect intercom, linear, slack.
